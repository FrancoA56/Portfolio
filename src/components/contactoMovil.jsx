...
function ContactameMovil() {
  const language = useSelector((state) => state.language);

  return (
    <div className="flex flex-col w-full text-light justify-center items-center dark:text-dark px-4">
      {language ? (
        <>
          <div className="w-full">
            <div className="text-center text-semititulo my-4">¡Contáctame!</div>
            <Formulario />
          </div>
        </>
      ) : (
        <div className="w-full">
          <div className="text-center text-semititulo my-4">Contact me!</div>
          <Formulario />
        </div>
      )}
...
export default ContactameMovil;
